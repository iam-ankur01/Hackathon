...
const Sidebar = () => {
  ...
  useEffect(() => {
    getDashboard()
      .then((data) => {
        setScore(Math.max(0, Math.min(100, Math.round(data?.hirescore || 0))));
        setHasAnalysis((data?.total_interviews || 0) > 0);
      })
      .catch(() => {
        setScore(0);
    ...
    <aside className="w-56 flex-shrink-0 hidden md:flex flex-col py-6 px-3" style={{background:'#fafafa', borderRight:'1px solid rgba(10,10,10,0.07)'}}>
      ...
      <div className="mt-auto">
        <div className="rounded-xl p-4" style={{background:'#0a0a0a', color:'#fafafa'}}>
          ...
          <p className="text-3xl font-display tracking-tightest" style={{fontWeight:700, color:'#ffffff'}}>{score}<span className="text-sm font-normal" style={{color:'#a1a1aa'}}>/100</span></p>
          <div className="mt-2" style={{height:6, borderRadius:3, background:'rgba(255,255,255,0.1)', overflow:'hidden'}}>
            <div style={{height:'100%', width:`${score}%`, background:'#6d28d9', borderRadius:3}} />
          </div>
          <p className="text-xs mt-1.5" style={{color:'#a1a1aa'}}>{hasAnalysis ? 'Based on valid analyses' : 'No valid analyses yet'}</p>
        </div>
      </div>
    </aside>
  );
