  ...
  const Field = ({ label, icon: Icon, field, type='text', placeholder }) => (
    <div>
      ...
      <div className="relative">
        ...
        <input type={type} className={`input-field pl-10 text-sm ${!edit?'opacity-70 cursor-default':''}`} value={form[field]} onChange={e=>setForm({...form,[field]:e.target.value})} readOnly={!edit} placeholder={placeholder} />
      </div>
    </div>
  );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-5">
          <motion.div initial={{opacity:0}} animate={{opacity:1}} className="card flex flex-col items-center text-center py-8">
            ...
            <h2 className="font-display font-bold text-white text-lg">{form.name}</h2>
            ...
            <div className="mt-5 relative">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="font-display font-bold text-2xl text-white">{hirescore}</span>
          ...
          <div className="card">
            ...
            <div className="space-y-2">
              {[{icon:Github,label:'GitHub',val:form.github},{icon:Linkedin,label:'LinkedIn',val:form.linkedin}].map((l,i)=>(
                <a key={i} href={`https://${l.val}`} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-sm text-textMuted hover:text-white p-2 rounded-lg hover:bg-white/5 transition-all">
        ...
        <div className="lg:col-span-2 space-y-5">
          <div className="card space-y-4">
            <h3 className="font-semibold text-white text-sm uppercase tracking-wide">Personal Information</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <Field label="Target Role" icon={Briefcase} field="role" />
              <Field label="College / Company" icon={FileText} field="college" />
            </div>
            <div>
          ...
          <div className="card space-y-4">
            <h3 className="font-semibold text-white text-sm uppercase tracking-wide">Profile Links & Documents</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field label="GitHub Username" icon={Github} field="github" placeholder="github.com/username" />
              <Field label="LinkedIn Profile" icon={Linkedin} field="linkedin" placeholder="linkedin.com/in/..." />
              <Field label="Gmail" icon={Mail} field="gmail" placeholder="you@gmail.com" />
            </div>
            <div>
              <label className="text-xs font-semibold text-textMuted uppercase tracking-wide mb-2 block">Resume</label>
              <div className="flex items-center gap-3 p-4 rounded-xl" style={{background:'rgba(255,255,255,0.03)',border:'1px dashed rgba(255,255,255,0.1)'}}>
                <FileText className="w-5 h-5 text-primary" />
                <div className="flex-1">
                  <p className="text-white text-sm font-medium">Onkar_Kulkarni_Resume.pdf</p>
          ...
          <div className="card">
            <h3 className="font-semibold text-white text-sm uppercase tracking-wide mb-3">Skills</h3>
            {edit
